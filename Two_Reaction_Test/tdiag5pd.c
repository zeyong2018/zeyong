...
#include "stdafx.h"
/*-------------------------------------------------------------------*/
/*Test 5 diagonal matrix solver diag5pd for sym., str. nonsing. matr.*/
/*-------------------------------------------------------------------*/
...
int main (int argc, char *argv[])
{
  ...
  WriteHead ("5 diagonal, strongly nonsingular symmetric matrices");

  if (scanf("%d", &n) < 1 || scanf ("%d", &ld) < 1 ||
      scanf("%d ", &ud) < 1)
  {
  ...
  for (j = 0; j < n; j++)
  {
    ...
    printf (FORMAT_LF, (REAL) dum1);
    printf (FORMAT_LF, (REAL) dum2);
  ...
  for (i = 0; i < n; i++)
  {
    ...
    rc = diag5pd (cas, n, d, ud1, ud2, b);
    if (rc)
    {
      LogError ("diag5pd", rc,  __FILE__, __LINE__);
      return 1;
    }
